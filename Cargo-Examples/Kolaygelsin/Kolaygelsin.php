...
namespace Modules\Cargo\Drivers\Kolaygelsin;
...
class Kolaygelsin extends ShipmentBase implements ShipmentInterface
{
    protected $config = [];
    protected $baseUrl = '';
    protected $tokenCacheKey = 'kolaygelsin_token';

    public function __construct($config)
    {
        $this->config = $config;
        $this->baseUrl = $config['api_url'] ?? 'https://api.kolaygelsin.com'; // Dokümantasyondan doğrulanacak
    ...
    protected function getToken()
    {
        ...
        if (empty($customerCode) || empty($password)) {
            Log::error('Kolaygelsin: API credentials eksik');
            return null;
        }
        ...
        try {
            ...
            Log::error('Kolaygelsin: Token alınamadı', ['response' => $response->body()]);
            return null;
        } catch (\Exception $e) {
            Log::error('Kolaygelsin: Token alma hatası', ['error' => $e->getMessage()]);
            return null;
        }
    }
    ...
    public function send($params, $products)
    {
        try {
            $token = $this->getToken();
            if (!$token) {
                return $this->shipmentResponse(503, 'Kolaygelsin API token alınamadı. Lütfen kargo ayarlarınızı kontrol edin.');
            }
            ...
            if ($response->successful()) {
                ...
                if (isset($responseData['IsSuccess']) && $responseData['IsSuccess']) {
                    ...
                    Log::error('Kolaygelsin: Gönderi oluşturma hatası', ['response' => $responseData]);
                    return $this->shipmentResponse(400, 'Gönderi oluşturulamadı: ' . $errorMessage);
                }
            } else {
                Log::error('Kolaygelsin: API hatası', ['status' => $response->status(), 'body' => $response->body()]);
                return $this->shipmentResponse(503, 'API hatası: ' . $response->status());
            }
        } catch (\Exception $e) {
            Log::error('Kolaygelsin: Gönderi oluşturma exception', ['error' => $e->getMessage(), 'trace' => $e->getTraceAsString()]);
    ...
    public function cancel($params)
    {
        try {
            $token = $this->getToken();
            if (!$token) {
                return $this->shipmentResponse(503, 'Kolaygelsin API token alınamadı.');
            }
            ...
            if ($response->successful()) {
                ...
                if (isset($responseData['IsSuccess']) && $responseData['IsSuccess']) {
                    ...
                    $errorMessage = $responseData['Message'] ?? 'Bilinmeyen hata';
                    Log::error('Kolaygelsin: İptal hatası', ['response' => $responseData]);
                    return $this->shipmentResponse(400, 'İptal edilemedi: ' . $errorMessage);
                }
            } else {
                Log::error('Kolaygelsin: İptal API hatası', ['status' => $response->status(), 'body' => $response->body()]);
                return $this->shipmentResponse(503, 'API hatası: ' . $response->status());
            }
        } catch (\Exception $e) {
            Log::error('Kolaygelsin: İptal exception', ['error' => $e->getMessage()]);
            return $this->shipmentResponse(503, 'Bir hata oluştu: ' . $e->getMessage());
        }
    }
        ...
        try {
            ...
            if ($response->successful()) {
                ...
                if (isset($responseData['IsSuccess']) && $responseData['IsSuccess']) {
                    ...
                    $errorMessage = $responseData['Message'] ?? 'Bilinmeyen hata';
                    Log::error('Kolaygelsin: Takip hatası', ['response' => $responseData]);
                    return $this->shipmentResponse(400, 'Takip yapılamadı: ' . $errorMessage);
                }
            } else {
                Log::error('Kolaygelsin: Takip API hatası', ['status' => $response->status(), 'body' => $response->body()]);
                return $this->shipmentResponse(503, 'API hatası: ' . $response->status());
            }
        } catch (\Exception $e) {
            Log::error('Kolaygelsin: Takip exception', ['error' => $e->getMessage()]);
            return $this->shipmentResponse(503, 'Bir hata oluştu: ' . $e->getMessage());
        }
    }
    ...
    public function getLabel($params)
    {
        try {
            ...
            Log::error('Kolaygelsin: Etiket alma hatası', ['status' => $response->status(), 'body' => $response->body()]);
            return $this->shipmentResponse(503, 'Etiket alınamadı.');
        } catch (\Exception $e) {
            Log::error('Kolaygelsin: Etiket alma exception', ['error' => $e->getMessage()]);
            return $this->shipmentResponse(503, 'Bir hata oluştu: ' . $e->getMessage());
        }
    }
}
